...
public class UserWorkspaceService {
	...
	public UserWorkspaceResponse workspaceSearchByUser(Long workspaceId, Long createdBy) {
		...
		if(null == workspaceId) {
			...
			list = userWorkspaceRepository.findByWorkspaceIdAndCreatedByAndIsDeleteOrderByCreatedDateDesc(workspaceId, createdBy,"N");
		}
		...
		if(!list.isEmpty()) {
			...
			for (Iterator iterator = list.iterator(); iterator.hasNext();) {
			    ...
			    ResponseEntity<SearchDetailsResponse> responseEntity = restTemplate.exchange(absUrl,
			    		HttpMethod.GET, new HttpEntity<Object>(headers),
			    		SearchDetailsResponse.class);
		...
		if(userWorkspaceEntity!=null )
			{
				...
				System.out.println("absUrl = "+ absUrl);
			   /* ResponseEntity<List<UserSubscription>> responseEntity = restTemplate.exchange(absUrl,
			    		HttpMethod.GET, new HttpEntity<Object>(headers),
			    		new ParameterizedTypeReference<List<UserSubscription>>() {});*/
				
			    ResponseEntity<UserSubscriptionList> responseEntity = restTemplate.exchange(absUrl,
			    		HttpMethod.GET, new HttpEntity<Object>(headers),UserSubscriptionList.class);
